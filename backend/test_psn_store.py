...
# ─── Format / medium (#164 Chunk C2) ───────────────────────────────────────


def test_medium_for_item_infers_from_sources():
...
def test_physical_media_label_resolves_from_platform():
    ps4 = models.Platform(name="PlayStation 4", media_type="disc")
    vita = models.Platform(name="PlayStation Vita", media_type="cartridge")
    psp = models.Platform(name="PlayStation Portable", media_type="umd")
    steam = models.Platform(name="Steam")  # digital-only, no media_type
    assert models.physical_media_label(ps4) == "Disc"
    assert models.physical_media_label(vita) == "Cartridge"
    assert models.physical_media_label(psp) == "UMD"
    # Unseeded / digital-only / missing platform → don't guess.
    assert models.physical_media_label(steam) == "Physical"
    assert models.physical_media_label(None) == "Physical"


_fmt_seq = [0]


def _entry_with(db, *, medium, media_type=None, membership=None, source="psn"):
    _fmt_seq[0] += 1
    n = _fmt_seq[0]
    user = models.User(name=f"u{n}", username=f"u{n}", password_hash="x", api_token=f"t{n}")
    db.add(user)
    db.flush()
    plat = models.Platform(name=f"Plat{n}", media_type=media_type)
    db.add(plat)
    db.flush()
    g = models.Game(title="G")
    db.add(g)
    db.flush()
    rel = models.GameRelease(
        game_id=g.id,
        ...
        platform_id=plat.id,
        source=source,
        external_id=f"X{plat.id}",
        raw_data={"membership": membership} if membership else {},
    )
    db.add(rel)
    db.flush()
    e = models.UserLibraryEntry(user_id=user.id, release_id=rel.id, import_source="psn_import", medium=medium)
    db.add(e)
    db.flush()
    return e


def test_extract_format_meta(db_session):
    from backend.pages_common import _extract_format_meta

    # Digital, owned outright → no service badge.
    m = _extract_format_meta(_entry_with(db_session, medium="digital"))
    assert m["format"] == "Digital" and m["service"] is None

    # Digital via PS Plus → Digital + the PS Plus badge (membership is not a format).
    m = _extract_format_meta(_entry_with(db_session, medium="digital", membership="PS_PLUS"))
    assert m["format"] == "Digital"
    assert m["service"]["label"] == "PS Plus"

    # Physical resolves the platform's media word.
    assert _extract_format_meta(_entry_with(db_session, medium="physical", media_type="disc"))["format"] == "Disc"
    assert _extract_format_meta(_entry_with(db_session, medium="physical", media_type="cartridge"))["format"] == "Cartridge"
    # Physical on a platform with no media_type → plain "Physical".
    assert _extract_format_meta(_entry_with(db_session, medium="physical"))["format"] == "Physical"

    # Unknown medium → no row at all.
    assert _extract_format_meta(_entry_with(db_session, medium=None))["format"] is None
