"""Library routes: the library list/grid page and its infinite-scroll pagination,
add / edit / delete / hide entries, IGDB metadata fetch + unlink, the detail pane,
metadata refresh, cover-override + auto-fetch artwork, and the card fragments.

Moved verbatim out of `pages.py` — no behaviour changes. The shared library query
builder (`_build_lib_query`) lives in `pages_common` because the home/tools
dashboards use it too; the steam/igdb/steamgriddb helpers stay as function-local
imports (as they were), so this module has no module-level dependency on them.
"""
...
def library_entry_detail(
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "entitlement_meta": _extract_entitlement_meta(entry),
