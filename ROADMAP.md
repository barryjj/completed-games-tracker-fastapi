...
- **Mass-select + bulk approve:** ✅ shipped 2026-07-08 for add_to_existing — a toggleable Select mode reveals checkboxes (hidden during normal review), 'Select all loaded' + bulk Confirm posts the batch through the same confirm path (dup-skip + Reopen linkage included). Bulk *create* deliberately not built: create_new confirms route through the prefilled add-game modal as a human quality gate, and bulk-creating would skip it — needs its own design if ever wanted
...
- **Import Review UI (`feat/historical-import`, functional, still being refined):** tabbed by proposed action (Add to existing / Create new / Needs review) with live counts, server-side search/platform/year/sort filters, click-row-to-preview, Edit + manual library-link actions, and a matcher engine rebuilt through extensive live testing against the real spreadsheet (1994+ candidates). Current matcher design (`importer.py`):
  - `_exact_match_entry` → `_prefix_match_entry` (colon/dash subtitle on the spreadsheet side) → `_library_prefix_match_entry` (subtitle on the library side only) → `_pool_fallback_entry` (SQL-narrowed pool of exactly one candidate) → `_collection_match_entry` (fallback only: spreadsheet's Collection column as a search hint, not a gate — finds a matching library entry by title, then checks its `parent_id` children)
  - All passes narrow candidates via a cheap SQL `ILIKE` substring search (`_search_pool`) before any Python-side token comparison — faster and safer than scanning the whole library
  - `_title_contains_remainder` requires the target phrase as a contiguous in-order token run, not bag-of-tokens (fixed a real bug where "Street Fighter II" matched "Street Fighter Alpha 2")
  - "Create new" confirm now actually creates the entry (opens the real add-game modal prefilled, auto-logs completions from the parsed rows) instead of a dead redirect
  - "Recheck matches" re-runs the matcher against all pending candidates without re-uploading; completion date precision (day/month/year) tracked end-to-end so historical rows with only a year or month don't render a fabricated "January 1, 2012"
  - `write_candidates`' confirmed-row dedup now checks *all* confirmed candidates for a title+platform (was only the first match), preventing a duplicate candidate from surviving re-upload after one sibling was already confirmed
