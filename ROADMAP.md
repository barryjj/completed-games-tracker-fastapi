...
- **Import Review UI (`feat/historical-import`, mostly done):** tabbed by proposed action (Add to existing / Create new / Needs review) with live counts, server-side search/platform/year/sort filters, click-row-to-preview, Edit + manual library-link actions, and a matcher engine rebuilt through extensive live testing against the real spreadsheet (1994+ candidates). Current matcher design (`importer.py`):
  ...
  - **Still open:** layout polish — tabs currently styled as a `btn-group`, inconsistent with the existing settings-page tab component; long lists bury the tab/filter bar with no way back to the top; card-view-as-detail-pane for Add to existing (scrapped mid-session, then user said maybe bring it back) undecided
